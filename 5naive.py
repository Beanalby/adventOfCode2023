...
class Game:
    ...
    def run(self):
        ...
        print("------------------------------")
        seedsAndRanges = zip(self.seeds[0::2], self.seeds[1::2])
        for (baseSeed, seedRange) in seedsAndRanges:
            count=0
            print("Running from {} for {:,} seeds:".format(baseSeed, seedRange))
            for seed in range(baseSeed, baseSeed+seedRange):
                location = self.apply(seed)
                lowestLocation = min(location, lowestLocation)
                # print("{:,}->{:,}".format(seed, location))
                if count % 1000000 == 0:
                    print("\n{}: {: 10,}: ".format(datetime.now().strftime("%Y:%m:%d %H:%M:%S"), count), end="")
                if count % 10000 == 0:
